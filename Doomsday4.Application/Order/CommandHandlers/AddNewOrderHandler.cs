...
public class AddNewOrderHandler(RentDbContext context) : IRequestHandler<AddNewOrder, Guid>
{
    ...
    {
        var newOrder = await context.Orders.AddAsync(new Domain.Models.Order(command.UserId, command.StartDateTime, command.EndDateTime, command.Сost, command.OrderStatus), cancellationToken);
